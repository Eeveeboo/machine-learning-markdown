...
fn generate_test_module(
    ...
    let forward_args: Vec<String> = param_names.iter().map(|name| format!("&{}", name)).collect();
...
impl CodegenTarget for CandleCodegen {
    ...
    fn generate(&self, graph: &Graph) -> Vec<GeneratedFile> {
        ...
        for b in &sorted {
            ...
            if let Some(ref init) = result.init {
                if let CandleInitOrString::Candle(ref candle_init) = init {
                    ...
                    let body = candle_init
                        .body
                        .replace(&format!("vb.pp(\"{}\")", b.id), &format!("weights.pp({})", snake_name));
        ...
        vec![GeneratedFile {
            path: format!("{}.candle.rs", class_name),
...
mod tests {
    ...
    fn test_name_and_extension() {
        ...
        assert_eq!(target.file_extension(), "candle.rs");
    }
    ...
    fn test_generate_simple_graph() {
        ...
        let file = &files[0];
        assert!(file.path.ends_with(".candle.rs"));
