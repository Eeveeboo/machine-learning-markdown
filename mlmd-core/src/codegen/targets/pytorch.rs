...
fn generate_pytorch_test(class_name: &str, sorted: &[Block]) -> Option<String> {
    ...
    test_lines.push(format!("    output = model({args})", args = input_args.join(", ")));
    test_lines.push(format!("    assert output.shape == torch.Size([{output_shape_str}])"));
    test_lines.push(String::new());
...
mod tests {
    ...
    fn test_generate_simple_graph() {
        ...
        assert!(test_file.content.contains("from Model_pytorch import Model"));
