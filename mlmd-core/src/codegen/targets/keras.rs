...
fn generate_keras_test(class_name: &str, sorted: &[Block]) -> Option<String> {
    ...
    test_lines.push("import tensorflow as tf".to_string());
    test_lines.push(format!("from {}_keras import build_model", class_name));
    test_lines.push(String::new());
    test_lines.push(String::new());
    test_lines.push("def test_build_model():".to_string());
    test_lines.push("    model = build_model()".to_string());
    for l in &input_lines {
        test_lines.push(l.clone());
    }
    test_lines.push(format!("    output = model({args}, training=False)", args = input_args.join(", ")));
    test_lines.push(format!("    assert output.shape == ({output_shape_str},)"));
    test_lines.push(String::new());
...
impl CodegenTarget for KerasCodegen {
    ...
    fn generate(&self, graph: &Graph) -> Vec<GeneratedFile> {
        ...
        let mut files = vec![GeneratedFile {
            path: format!("{}_keras.py", model_name),
            content,
        }];

        if let Some(tc) = test_content {
            files.push(GeneratedFile {
                path: format!("{}_keras_test.py", model_name),
                content: tc,
            });
        }
...
mod tests {
    ...
    fn test_name_and_extension() {
        ...
        assert_eq!(target.file_extension(), "_keras.py");
    }
    ...
    fn test_generate_simple_graph() {
        ...
        assert!(model_file.path.ends_with("_keras.py"));
        assert!(model_file.content.starts_with("# This file was generated by mlmd. Do not edit."));
        ...
        assert!(test_file.path.ends_with("_keras_test.py"));
        assert!(test_file.content.starts_with("# This file was generated by mlmd. Do not edit."));
        assert!(test_file.content.contains("from Model_keras import build_model"));
