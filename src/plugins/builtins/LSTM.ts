...
export const LSTM: BlockPlugin = {
  ...
  codegen: {
    pytorch(block, inputVars, outputVars) {
      ...
      return {
        init: `self.${block.id} = nn.LSTM(${inputSize}, ${hidden}, num_layers=${layers}, batch_first=True)`,
    ...
    keras(block, inputVars, outputVars) {
      const hidden =
        ...
        0;
      return {
        forward: `${outputVars[0]} = keras.layers.LSTM(${hidden}, return_sequences=True)(${inputVars[0]})`,
      };
    },
    ...
    candle(block, inputVars, outputVars) {
      ...
      return {
        init: {
          field: `${block.id}: candle_nn::LSTM`,
          body: `let ${block.id} = candle_nn::lstm(${inputSize}, ${hidden}, candle_nn::LSTMConfig::default(), vb.pp("${block.id}"))?;`,
        },
