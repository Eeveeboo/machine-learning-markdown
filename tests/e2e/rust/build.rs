...
fn main() {
    // The manifest directory is where Cargo.toml lives: tests/e2e/rust/
    let manifest_dir = env::var("CARGO_MANIFEST_DIR").unwrap();
    let examples_dir = Path::new(&manifest_dir).join("../../../examples/");

    // Canonicalize to absolute path
    let examples_dir = fs::canonicalize(&examples_dir)
        .expect("failed to canonicalize examples directory");
    ...
    for entry in &entries {
        ...
        if path.extension().map_or(false, |ext| ext == "rs") {
            ...
            let absolute_path = fs::canonicalize(&path)
                .expect("failed to canonicalize example file path");
