...
// GRU block.
//
// Port of src/plugins/builtins/GRU.ts
// ---------------------------------------------------------------------------
...
impl BlockDef for GRUBlockDef {
    ...
    fn infer_shape(
        ...
        if inputs.is_empty() {
            return Err("GRU requires an input".to_string());
        }
    ...
    fn param_count(
        ...
        let first_layer = 3 * (in_size * h + h * h + 2 * h);
        let extra_layer = if l > 1 { (l - 1) * 3 * (h * h + h * h + 2 * h) } else { 0 };
...
pub fn register() {
    register_block(Box::new(GRUBlockDef));

    register_block_codegen("GRU", "pytorch", pytorch_codegen);
    register_block_codegen("GRU", "keras", keras_codegen);
    register_block_codegen("GRU", "candle", candle_codegen);
}
...
fn pytorch_codegen(
    ...
    BlockCodegenResult {
        init: Some(CandleInitOrString::Plain(format!(
            "self.{} = nn.GRU({}, {}, num_layers={}, batch_first=True)",
...
fn keras_codegen(
    ...
    BlockCodegenResult {
        init: None,
        forward: format!(
            "{} = keras.layers.GRU({}, return_sequences=True)({})",
            output_vars.first().map(|s| s.as_str()).unwrap_or("?"),
            hidden,
            input_vars.first().map(|s| s.as_str()).unwrap_or("?"),
        ),
...
fn candle_codegen(
    ...
    BlockCodegenResult {
        init: Some(CandleInitOrString::Candle(CandleInit {
            field: format!("{}: candle_nn::GRU", block.id),
            body: format!(
                "let {} = candle_nn::gru({}, {}, candle_nn::GRUConfig::default(), vb.pp(\"{}\"))?;",
...
mod tests {
    ...
    #[test]
    fn test_gru_block_def() {
        let def = GRUBlockDef;
        assert_eq!(def.name(), "GRU");
