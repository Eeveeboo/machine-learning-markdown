...
//
// Port of src/plugins/builtins/Tanh.ts
// ---------------------------------------------------------------------------
...
impl Plugin for Tanh {
    ...
    fn infer_shape(
        ...
        if inputs.is_empty() {
            return Err("Tanh requires an input".to_string());
        }
...
mod tests {
    ...
    #[test]
    fn test_tanh_block_def() {
        let def = Tanh;
        assert_eq!(def.name(), "Tanh");
