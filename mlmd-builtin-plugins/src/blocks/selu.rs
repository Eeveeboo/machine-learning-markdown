// ---------------------------------------------------------------------------
// SELU activation block.
//
// Port of src/plugins/builtins/SELU.ts
// ---------------------------------------------------------------------------
...
impl Plugin for SELU {
    ...
    fn infer_shape(
        ...
        if inputs.is_empty() {
            return Err("SELU requires an input".to_string());
        }
...
mod tests {
    ...
    #[test]
    fn test_selu_block_def() {
        let def = SELU;
        assert_eq!(def.name(), "SELU");
